...
RepoShare 提供了一套完整的模板系统，涵盖了多种设计风格和布局方案。每个模板都经过精心设计，既能突出项目的核心信息，又能体现独特的视觉美感。用户可以根据项目的性质和个人偏好，选择最适合的模板风格，并通过显示项、布局方向、导出格式和分辨率等选项调整最终效果。
...
RepoShare 不仅提供了丰富的预设模板，还支持控制 Star、Fork、主页链接、作者信息等展示内容，并可选择横向或竖向布局、PNG 或 JPEG 格式以及不同导出倍率。这些选项能覆盖 README 展示、社交分享和项目文档等常见场景。
...
### 像素模板 (Pixel)
Pixel 模板采用复古像素风格，为项目展示增加鲜明的游戏化视觉特征，并支持作者头像和主页二维码展示。
...
### 水墨模板 (Ink)
Ink 模板使用柔和的纸张质感和水墨风格排版，适合偏文档、工具或个人项目的分享图。
...
### UI 框架
应用壳层采用 Tailwind CSS v4 和 shadcn/ui 风格的本地组件构建，便于通过 utility class 与设计 token 持续维护。Ant Design 目前仅保留通知和二维码等少量能力，模板导出样式则继续独立维护，避免界面重构影响图片生成结果。
...
RepoShare 的应用壳层采用 Tailwind CSS v4 + shadcn/ui 风格组件维护，目标是让主界面保持类似 PostSpark 的轻量工具工作台质感，同时避免继续扩张手写长 CSS。

- **选型**：Tailwind CSS v4 通过 Vite 插件接入；shadcn/ui 组件以本地源码形式放在 `src/components/ui/*`，共享工具函数为 `src/lib/utils.ts`。
- **原则**：应用壳层（Header、Sidebar、ControlPanel、MainContent）优先使用 shadcn 风格组件和 Tailwind utility class；模板导出图片自身样式继续保留在 `src/templates/*/style.css`，避免影响导出稳定性。
- **迁移边界**：本轮不完全移除 Ant Design。`notification` 和模板中的 `QRCode` 仍可继续使用 AntD；后续如需彻底去 AntD，应作为独立任务处理。
- **验收命令**：每次 UI 壳层调整后运行 `npm run lint`、`npm run build`、`npm audit`，并检查桌面与窄屏下顶部栏、左右面板、中心画布没有重叠。
...
interface TemplateConfig {
  ...
  component: React.ComponentType<TemplateProps>; // 模板的组件
  supportsAuthorAvatar?: boolean; // 是否支持作者头像开关
}
```
