...
### 前端框架
RepoShare 使用 React 19.x 和 TypeScript 构建，应用状态、响应式工作台和模板预览均采用函数式组件与 Hooks 维护。

### UI 框架
应用壳层采用 Tailwind CSS v4、Radix/shadcn 风格本地组件和 Liquid Glass 控制层构建。Ant Design 6 仅保留通知、图标和二维码等能力，模板导出样式继续独立维护。

### 开发工具
项目使用 TypeScript 6、Vite 8 和 ESLint 10，并通过设计 token 与共享布局常量约束应用壳层。
...
应用壳层使用 Tailwind CSS v4 和 shadcn/ui 风格组件维护。布局、间距、主题、组件状态和响应式行为以 [RepoShare 设计系统](docs/design-system.md) 为准；模板导出样式继续独立维护在 `src/templates/*/style.css`。
